...
// Definindo tamanhos específicos por categoria
const sizesByCategory = {
  "Acessórios": ["UNICO"], // Mochilas, bonés, meias - tamanho único
  "Bermuda & Shorts": ["P", "M", "G", "GG"],
  "Calças": ["P", "M", "G", "GG"],
  "Camisetas": ["P", "M", "G", "GG"],
  "Jaquetas & Moletons": ["P", "M", "G", "GG"],
  "Tênis": ["38", "39", "40", "41", "42", "43", "44"], // Numeração de calçados
} as const;
...
async function main() {
  ...
  try {
    ...
    for (const productData of products) {
      ...
      // Obter tamanhos adequados para a categoria
      const availableSizes = sizesByCategory[productData.categoryName as keyof typeof sizesByCategory];
      ...
      for (const variantData of productData.variants) {
        ...
        // Criar variantes para cada tamanho disponível
        for (const size of availableSizes) {
          const variantId = crypto.randomUUID();
          const variantSlug = generateSlug(`${productData.name}-${variantData.color}-${size}`);

          console.log(`  🎨 Criando variante: ${variantData.color} - Tamanho ${size}`);
    ...
    // Calcular o total de variantes criadas
    const totalVariants = products.reduce((acc, product) => {
      const categoryName = product.categoryName as keyof typeof sizesByCategory;
      const sizesCount = sizesByCategory[categoryName].length;
      return acc + (product.variants.length * sizesCount);
    }, 0);
